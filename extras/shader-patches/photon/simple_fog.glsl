...
#ifndef FOG_END_UNIFORM
#define FOG_END_UNIFORM
...
float border_fog(vec3 scene_pos, vec3 world_dir) {
	// RPG Mechanics: drive chunk-edge fog with fogEnd (aimed at the custom border).
	// Falls back to far / DH distance when fogEnd is not shrunk.
#ifndef DISTANT_HORIZONS
	float fog_radius = max(min(far, fogEnd), 1.0);
	float fog = cubic_length(scene_pos.xz) / fog_radius;
	      fog = exp2(-8.0 * pow8(fog));
#else
    float fog_radius = max(min(float(dhRenderDistance), fogEnd), 1.0);
    float fog = length(scene_pos.xz) / fog_radius;
